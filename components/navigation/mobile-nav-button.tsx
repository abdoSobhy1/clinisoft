...
export default function MobileNavButton(props: Props) {
    return (
        <motion.div variants={buttonVariants} initial="initial" animate="open" exit="initial">
            <Link href={props.href} className={`flex items-center gap-2 text-white text-2xl opacity-70 font-medium justify-center px-2 uppercase transition duration-300  hover:opacity-100  ${props.isActive ? 'opacity-100' : ''}`} onClick={props.onClick} >
                {props.label}
