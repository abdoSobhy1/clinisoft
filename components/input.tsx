...
export default function Input({ type, label, form, isRequired, error }: {
    ...
    return (
        <div className="form-group">
            <label htmlFor={label.toLocaleLowerCase()} className="font-bold fs-var-xl lg:fs-var-base">{label} {isRequired && <span className="text-red-500 font-medium">*</span>}</label>
            ...
            {error && <p className="text-red-500 fs-var-sm">{error}</p>}
        </div>
    )
}
